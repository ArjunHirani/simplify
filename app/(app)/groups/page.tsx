    ...
    <>
      ...
      <div style={{ maxWidth: 900, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          <div>
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 700, color: "#fff", margin: "0 0 4px" }}>
              Groups
            </h1>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: "0 4px 16px rgba(255,79,121,0.3)",
            }}
          >
        ...
        {/* Error */}
        {error && (
          <div style={{ background: "rgba(255,107,107,0.1)", border: "1px solid rgba(255,107,107,0.2)", borderRadius: 12, padding: "16px", color: "#FF6B6B", fontSize: 13, marginBottom: 20 }}>
            {error}
          </div>
        )}
        ...
        {!loading && groups.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 20px", background: "#17172B", borderRadius: 14, border: "0.5px solid rgba(255,255,255,0.07)" }}>
            ...
            <button
              onClick={() => setShowCreate(true)}
              style={{ background: "#FF4F79", border: "none", borderRadius: 12, padding: "12px 24px", color: "#fff", fontSize: 14, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", cursor: "pointer" }}
            >
              Create first group
            </button>
          </div>
...
function GroupCard({ group }: { group: Group }) {
  ...
  return (
    <div className="group-card" style={{
      background: "#17172B", borderRadius: 14, padding: 16,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <div>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 14, fontWeight: 600, color: "#fff", marginBottom: 2 }}>{group.name}</div>
          <div style={{ fontSize: 11, color: "#A1A1AA" }}>{group.members} members{group.description ? ` · ${group.description}` : ""}</div>
        </div>
      </div>
...
function CreateGroupModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const inputStyle = { width: "100%", padding: "11px 14px", background: "#12121F", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#fff", fontSize: 14, fontFamily: "'DM Sans', sans-serif", outline: "none", boxSizing: "border-box" as const };
  const labelStyle = { display: "block" as const, fontSize: 12, fontWeight: 500 as const, color: "#A1A1AA", marginBottom: 6 };
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#17172B", borderRadius: 18, border: "0.5px solid rgba(255,255,255,0.1)", padding: 24, width: "100%", maxWidth: 440, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <form onSubmit={handleSubmit} noValidate>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {EMOJIS.map((e) => (
                <button key={e} type="button" onClick={() => setEmoji(e)} style={{
                  ...
                  border: emoji === e ? "1.5px solid #FF4F79" : "1px solid rgba(255,255,255,0.08)",
                  background: emoji === e ? "rgba(255,79,121,0.1)" : "#12121F",
                  cursor: "pointer",
                }}>{e}</button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input type="text" style={inputStyle} placeholder="e.g. Flat 4B" value={name} onChange={e => setName(e.target.value)} autoFocus />
          </div>

          <div style={{ marginBottom: 14 }}>
            <label style={labelStyle}>Description (optional)</label>
            <input type="text" style={inputStyle} placeholder="e.g. Rent & utilities" value={desc} onChange={e => setDesc(e.target.value)} />
          </div>

          <div style={{ marginBottom: 20 }}>
            <label style={labelStyle}>Invite members by email (comma separated)</label>
            <input type="text" style={inputStyle} placeholder="rohan@example.com, priya@example.com" value={emails} onChange={e => setEmails(e.target.value)} />
            <p style={{ fontSize: 11, color: "#52526E", marginTop: 5 }}>Only existing Simplify users will be added.</p>
          </div>

          <div style={{ display: "flex", gap: 10 }}>
            <button type="button" onClick={onClose} style={{ padding: "12px 18px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "#A1A1AA", fontSize: 14, fontWeight: 500, fontFamily: "'DM Sans', sans-serif", cursor: "pointer" }}>Cancel</button>
